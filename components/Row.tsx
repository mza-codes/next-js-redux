...
export default function Row({ movies, title }: RowProps) {
    return (
        <section className="flex flex-col flex-wrap gap-2 max-w-[98vw] mx-auto">
            ...
            <div
                style={{ scrollPadding: "1.5rem" }}
                className="w-full px-6 py-4 overflow-x-scroll overflow-y-hidden flex flex-row gap-4 no-scrollbar hrow"
            >
                {movies?.map((movie) => (
                    <div
                        key={movie?.id}
                        className="img min-w-[200px] max-w-[220px] min-h-[200px] max-h-[300px] transition-transform duration-700 hover:scale-110 hover: cursor-pointer"
                    >
                        <img
